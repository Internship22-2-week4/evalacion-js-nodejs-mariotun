export default class AuthController {
  ...
  async authenticationUser (user) {
    try {
      ...
      const result = await this._services.findByAttribute('user', '_email', user.email)
      //console.log('--->Controller: ', result)
      if (result !== null) {
        const resltComparePassword = await this._comparePassword(user.password, result[0]._password)
